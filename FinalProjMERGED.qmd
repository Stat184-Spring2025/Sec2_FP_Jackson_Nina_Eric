...
url2 <- "https://gist.githubusercontent.com/jgasperackpsu/2c926a834c47eb5fc78d0626e7ff05bb/raw/140c327396394e8528acb4caaaf3d34d3a500fe5/Players.csv"

nba <- readr::read_csv(url2)

```

Both .csv files will be loaded through these links so everything should run properly regardless of if you have the files downloaded on your machine or not. The athletes data set was scraped and wrangled by @olympicData, and the nba data set was scraped and wrangled by @nbaData.
...
After cleaning the data up a bit, it is time to make a couple of visualizations to see if there are any obvious trends or outliers in the data set. The cleaning of the data can be seen in @sec-codeAppend12.
...
```{r}
#| label: ageFrequencyPlot
#| include: false
#| echo: true

# tile plot with linear scale
tilePlot <- ggplot(age_year_freq, aes(x = Year, y = Age, fill = Freq)) +
...
```{r fig-ageFrequencyPlot, echo=FALSE}
...
```{r fig-olympicsnogaps, echo=FALSE}
...
#| fig-align: center
#| fig-cap: "Basketball Male Height vs Weight; Grouped by Medal Count"
#| fig-cap-location: top
maleBB
```
...
#| fig-align: center
#| fig-cap: "Basketball Female Height vs Weight; Grouped by Medal Count"
#| fig-cap-location: top
femaleBB
```
...
### Age Analysis {#sec-codeAppend11}
```{r, ref.label= c("wranglingForAgeAnalysis")}
#| label: codeAppend11
#| echo: true
#| eval: false
```

### Wrangling Data to a Country-Year Case {#sec-codeAppend13}
```{r, ref.label= c("wrangleyearcase")}
#| label: codeAppend13
#| echo: true
#| eval: false
```

### Wrangling Data to Isolate for Seasons {#sec-codeAppend14}
```{r, ref.label= c("wrangleseasons")}
#| label: codeAppend14
#| echo: true
#| eval: false
```
...
### Mean Age Over Time {#sec-codeAppend12}
```{r, ref.label= c("meanAgePlot")}
#| label: codeAppend12
#| echo: true
#| eval: false
```

### Olympic Participation Over Time {#sec-codeAppend15}

```{r, ref.label= c("olympicsgaps","olympicsnogaps")}
#| label: codeAppend15
#| echo: true
#| eval: false
```

### Summer vs. Winter {#sec-codeAppend16}
```{r, ref.label= c("summergames","wintergames")}
#| label: codeAppend16
#| echo: true
#| eval: false
```
